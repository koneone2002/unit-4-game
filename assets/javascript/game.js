$(document).ready(function() {
  ...
  // create random numbers for jeweles from 1-12 - the jewel will keep the random number until win/loss, but will reset to a new value upon initializeGame()

  function makeJewelsRandom() {
    jewels = [];
    while (jewels.length < 4) {
      var randomJewelNum = Math.floor(Math.random() * 12) + 1;
      if (jewels.indexOf(randomJewelNum) === -1) {
        jewels.push(randomJewelNum);
      }
      //   console.log(jewels);
    }
    jewelBlue = jewels[0];
